...
@Service
@Transactional
public class DtapServiceImpl implements DTAPService{
	...
	public DTAHelper calculateL2(int ocu, int age, int term, double intrat, String sex, Date chedat, double loanamt,
		...
		for (int i = 1; i <= term; ++i) {
			...
			try {
				premium = (reduction.multiply(new BigDecimal(rateCardDTA.getRate()))).divide(new BigDecimal(1000), 0,
						BigDecimal.ROUND_HALF_UP);
			} catch (Exception e) {
				throw new NullPointerException("Error at DTAP Premium Calculation");
			}
	...
	@Override
	public QuotationQuickCalResponse getCalcutatedDtap(QuotationCalculation quotationCalculation) throws Exception {
	...
	public HashMap<String, Object> saveQuotation(QuotationCalculation calculation, InvpSaveQuotation _invpSaveQuotation,
		...
		if (productDao.findByProductCode("DTAP").getActive() == 0) {
		...
		QuotationQuickCalResponse calResp = getCalcutatedDtap(calculation);
		...
		Products products = productDao.findByProductCode("DTAP");
