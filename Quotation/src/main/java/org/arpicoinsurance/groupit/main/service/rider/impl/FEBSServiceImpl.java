...
public class FEBSServiceImpl implements FEBSService{
	...
	public BigDecimal calculateFEBS(Integer age, Integer term, Date chedat, Double ridsumasu, String payFrequency,
		...
		if(payFrequency.equalsIgnoreCase("S")){
			...
			premiumFEBS = ((new BigDecimal(rateCardATFESC.getRate()).multiply(new BigDecimal(ridsumasu)).divide(new BigDecimal(1000), 6, RoundingMode.HALF_UP)).divide(new BigDecimal(new CalculationUtils().getPayterm(payFrequency)), 10, RoundingMode.HALF_UP)).multiply(new BigDecimal(relief)).setScale(0, RoundingMode.HALF_UP);  
		}
