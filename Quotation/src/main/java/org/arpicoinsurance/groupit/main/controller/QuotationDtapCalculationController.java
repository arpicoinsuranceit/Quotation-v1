...
import org.arpicoinsurance.groupit.main.service.DTAPService;
...
@RestController
@CrossOrigin(origins = "*")
public class QuotationDtapCalculationController {
	@Autowired
	private DTAPService dtaService;
	...
	@RequestMapping(value = "/quoDtapCal", method = RequestMethod.POST)
	public ResponseEntity<Object> calculateQuotation(@RequestBody QuotationCalculation calculation) {
		...
		try {
			...
			logs.setOperation("calculateQuotation : QuotationDtaCalculationController");
			try {
	...
	@RequestMapping(value = "/quoDtapsave/{id}", method = RequestMethod.POST)
	public ResponseEntity<Object> saveDtap(@RequestBody InvpSaveQuotation _invpSaveQuotation, @PathVariable Integer id)
			throws Exception {
		...
		try {
			...
			logs.setDate(new Date());
			logs.setHeading("Error");
			logs.setOperation("saveDta : QuotationDtaCalculationController");
			logs.setUserId(id);
			try {
				logService.saveLog(logs);
			} catch (Exception e1) {
	...
	@RequestMapping(value = "/quoDtapEdit/{userId}/{qdId}", method = RequestMethod.POST)
	public ResponseEntity<Object> editDtap(@RequestBody InvpSaveQuotation _invpSaveQuotation,
		...
		try {
			...
			logs.setOperation("editDta : QuotationDtaCalculationController");
			try {
				logService.saveLog(logs);
			} catch (Exception e1) {
				// System.out.println("... Error Message for Operation ...");
	...
	@RequestMapping(value = "/quoDtapEditUnderwrite/{token}/{qdId}", method = RequestMethod.POST)
	public ResponseEntity<Object> editDtapUnderwrite(@RequestBody InvpSaveQuotation _invpSaveQuotation,
		...
		try {
			...
			logs.setOperation("editDtaUnderwrite : QuotationDtaCalculationController");
			try {
				logService.saveLog(logs);
			} catch (Exception e1) {
				System.out.println("... Error Message for Operation ...");
