...
public class TPDDTAPLServiceImpl implements TPDDTAPLService {
	...
	@Override
	public BigDecimal calculateTPDDTAPL(int age, int term, double intrat, String sex, Date chedat, double loanamt, double occupation_loding)
			throws Exception {
		...
		for (int i = 1; i <= term; ++i) {
            ...
            BigDecimal premium = ((reduction.multiply(new BigDecimal(rateCardTPDDTA.getRate()))).divide(new BigDecimal(1000), 8, BigDecimal.ROUND_HALF_UP)).multiply(new BigDecimal(1.2)).setScale(0, RoundingMode.HALF_UP);
