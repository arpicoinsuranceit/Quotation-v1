...
public class TPDDTASServiceImpl implements TPDDTASService {
	...
	@Override
	public BigDecimal calculateTPDDTAS(int age, int term, double intrat, String sex, Date chedat, double loanamt) throws Exception {
		...
		for (int i = 1; i <= term; ++i) {
			...
			try {
				premium = ((reduction.multiply(new BigDecimal(rateCardTPDDTAS.getRate()))).divide(new BigDecimal(1000),
						8, BigDecimal.ROUND_HALF_UP)).multiply(new BigDecimal(0.85)).setScale(0, RoundingMode.HALF_UP);
			} catch (Exception e) {
