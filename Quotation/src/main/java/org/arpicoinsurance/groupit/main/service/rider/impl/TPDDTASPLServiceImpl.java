...
public class TPDDTASPLServiceImpl implements TPDDTASPLService {
	...
	@Override
	public BigDecimal calculateTPDDTASPL(int age, int term, double intrat, String sex, Date chedat, double loanamt, double occupation_loding)
			throws Exception {
		...
		for (int i = 1; i <= term; ++i) {
         ...
         // ((@loan_reduction@*@rate@/1000)*1.2)*0.85
            BigDecimal premium = (((reduction.multiply(new BigDecimal(rateCardTPDDTAS.getRate()))).divide(new BigDecimal(1000), 8, BigDecimal.ROUND_HALF_UP)).multiply(new BigDecimal(1.2))).multiply(new BigDecimal(0.85)).setScale(0, RoundingMode.HALF_UP);
		...
		premiumTPDDTASPL = premiumTPDDTASPL.multiply(new BigDecimal(occupation_loding)).setScale(0, RoundingMode.HALF_UP);
